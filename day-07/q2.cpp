...
int main() {
    ...
    for (string line : input::readLines("input.txt")) {
        ...
        hands.push_back(Hand(parts.at(0), stoi(parts.at(1)), true));
    }
...
int calculateScore(const Hand& hand) {
    ...
    if (kinds[KIND_FOUR].size() == 1) {
        if (hand.numberOfJokers == 1) return 7;
        else return 6;
    }
    if (kinds[KIND_THREE].size() == 1 && kinds[KIND_TWO].size() == 1) return 5;
    if (kinds[KIND_THREE].size() == 1) {
        if (hand.numberOfJokers == 2) return 7;
        if (hand.numberOfJokers == 1) return 6;
        else return 4;
    }
    if (kinds[KIND_TWO].size() == 2) {
        if (hand.numberOfJokers == 1) return 5;
        else return 3;
    }
    if (kinds[KIND_TWO].size() == 1) {
        if (hand.numberOfJokers == 3) return 7;
        if (hand.numberOfJokers == 2) return 6;
        if (hand.numberOfJokers == 1) return 4;
        else return 2;
    }

    if (hand.numberOfJokers == 5) return 7;
    if (hand.numberOfJokers == 4) return 7;
    if (hand.numberOfJokers == 3) return 6;
    if (hand.numberOfJokers == 2) return 4;
    if (hand.numberOfJokers == 1) return 2;
